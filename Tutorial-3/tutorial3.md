...
```
! cp /home/dlang/lecture.ipynb tutorial3.ipynb
```
...
Try pulling out the position and velocity values and plotting the
kinetic and potential energies.  Is the sum of kinetic and
...
because of the packing and unpacking position and velocity.

Once you've got that, test it for the same problem with ran RK4 on
above.  You should find that it is *stable* over long periods -- the
planet doesn't shoot off into the outer Solar system, even if you run
it for a long time.

Pull out the position and velocity, as before, and plot the total
energy.
...
energy*.  I was, ahem, *not quite right* about that.  Instead, it
preserves a slightly more complicated thing: *phase-space volume*.
